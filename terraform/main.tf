...
resource "null_resource" "backend" {
    triggers = {
      instance_id = module.backend.id # this will be triggerd everytime instance is created
    }
    ...
    provisioner "remote-exec" {
        inline = [ 
            "chmod +R 777 /tmp/${var.common_tags.Component}.sh",
...
resource "aws_lb_listener_rule" "backend" {
  ...
  condition {
    host_header {
      values = ["backend.app-${var.environment}.${var.zone-name}"]
    }
  }
}